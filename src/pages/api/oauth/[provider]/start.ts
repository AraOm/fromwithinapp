...
export default async function handler(
  ...
  try {
    ...
    if (provider === "fitbit") {
      ...
      auth.searchParams.set(
        "redirect_uri",
        process.env.FITBIT_REDIRECT_URI as string
      );
    ...
    if (provider === "oura") {
      ...
      auth.searchParams.set(
        "client_id",
        process.env.OURA_CLIENT_ID as string
      );
      auth.searchParams.set(
        "redirect_uri",
        process.env.OURA_REDIRECT_URI as string
      );
    ...
    // 🟣 GOOGLE FIT
    // accept both /api/oauth/google/start and /api/oauth/googlefit/start
    if (provider === "google" || provider === "googlefit") {
      const auth = new URL("https://accounts.google.com/o/oauth2/v2/auth");
      auth.searchParams.set("response_type", "code");
      auth.searchParams.set(
        "client_id",
        process.env.GOOGLE_FIT_CLIENT_ID as string
      );
      auth.searchParams.set(
        "redirect_uri",
        process.env.GOOGLE_FIT_REDIRECT_URI as string
      );
    ...
    // 🟣 SAMSUNG – for now, just send them to a "coming soon" page
    if (provider === "samsung") {
      // You can change this path to whatever you prefer
      return res.redirect(
        302,
        `${base}/onboarding?wearable=samsung&status=coming-soon`
      );
    }
