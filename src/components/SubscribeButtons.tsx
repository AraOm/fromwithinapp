...
export function SubscribeButtons({ userId, email }: { userId?: string; email?: string }) {
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      >
        {loading === "golden" ? "Starting trial…" : "Start 7-Day Mystic Path → Golden Lion ($11.11/mo)"}
      </button>
      ...
      <button
        ...
        className="rounded-2xl px-5 py-3 font-medium shadow-lg bg-white/5 hover:bg-white/10 backdrop-blur border border-white/10"
      >
        {loading === "infinity" ? "Opening checkout…" : "∞ Path — Lifetime Access ($333)"}
      </button>
    </div>
  );
}
