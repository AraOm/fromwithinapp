...
// If you use shadcn Dialog, import it; otherwise we can switch to a simple div/modal later:
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
const PROVIDERS: { key: ProviderKey; label: string; description: string; icon: React.ReactNode; available: boolean; }[] = [
  {
    key: "fitbit",
    label: "Fitbit",
    description: "Sync heart rate, HRV, steps, and movement.",
    icon: <Activity className="h-4 w-4" />,
    available: true, // we'll wire this up first
  },
  {
    key: "oura",
    label: "Oura Ring",
    ...
    available: true, // second provider we’ll wire
  },
  {
    key: "apple_health",
  ...
  },
  {
    ...
    description: "Coming soon – connect Android-based health data.",
...
export const ConnectWearable: React.FC = () => {
  ...
  const handleConnect = (provider: ProviderKey, available: boolean) => {
    ...
    // For now, we just redirect. We'll create these API routes next:
    if (provider === "fitbit") {
      ...
      <DialogContent className="max-w-md border border-slate-800 bg-slate-950/90 backdrop-blur-xl">
        <DialogHeader>
          ...
          <DialogDescription className="text-xs text-slate-400">
            FromWithin translates your heart, breath, and sleep into energy insights.
            Choose the wearable (or app) you use most.
          </DialogDescription>
        </DialogHeader>
        ...
        <p className="mt-4 text-[11px] leading-relaxed text-slate-500">
          We’ll never share your health data. It’s encrypted and used only to help
          you understand your nervous system, emotional energy, and daily rhythms.
        </p>

        {isOpening && (
          <p className="mt-2 text-[11px] text-slate-400">
            Opening {isOpening === "fitbit" ? "Fitbit" : isOpening === "oura" ? "Oura" : "provider"}…
          </p>
        )}
      </DialogContent>
    </Dialog>
